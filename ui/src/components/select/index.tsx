...
export function SelectTrigger({
  ...
  return (
    <BaseSelect.Trigger
      ...
      data-size={size}
      className={cn(
        "[&_svg:not([class*='text-'])]:text-text-muted hover:text-text hover:bg-button-border active:bg-background focus-visible:border-button-border focus-visible:ring-button-border/50 ring-danger/50 border-danger bg-button flex w-fit items-center justify-between gap-2 rounded-md border border-button-border px-3 py-2 text-sm whitespace-nowrap shadow-xs transition-[color,box-shadow] outline-none select-none focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50 data-[size=default]:h-9 data-[size=sm]:h-8 *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-2 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    >
